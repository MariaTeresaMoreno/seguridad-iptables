...
#Contra ataques
#Detectar paquetes no válidos
iptables -t mangle -A PREROUTING -m conntrack --ctstate INVALID -j DROP
#Bloquear nuevos paquetes que no sean SYN
iptables -t mangle -A PREROUTING -m conntrack --ctstate INVALID -j DROP
#Detecta si MSS no tiene un valor correcto
...
# FIN Scan
ptables -A INPUT -p tcp --tcp-flags FIN,ACK FIN -j DROP 
...
#Limite de conexiones del cliente a 20/60s
